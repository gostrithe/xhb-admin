...
const queryEmployee = async () => {
  ...
  if (res) {
    for (let i in Employee_Information) {
      console.log(Employee_Information.value[i]);
    }
    // Employee_Information.value.map()
    // Employee_Information.value.employeeNme = res.employeeNme;
  }
};
