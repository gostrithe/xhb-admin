    ...
    <el-tab-pane label="潜在学员" name="first">
      ...
      >
        ...
        <el-form-item>
          ...
          <el-button type="primary" class="btn" @click="queryPotential_trainees"
            >查询</el-button
          >
          <el-button
...
//按类型查询潜在学员
const queryPotential_trainees = async () => {
  const res = await getStudents();
  potentialStudent.value = res!.data;
  const data = potentialStudent.value.filter(function (item) {
    if (
      item.name === AddPotential_trainees.name ||
      item.phone === AddPotential_trainees.phone ||
      item.follow === AddPotential_trainees.follow ||
      item.source === AddPotential_trainees.source ||
      item.level === AddPotential_trainees.level ||
      item.birthday === AddPotential_trainees.birthday ||
      item.remark === AddPotential_trainees.remark
    ) {
      return item.name || item.phone || item.follow || item.source ||item.level||item.birthday||item.remark;
    }
  });
  potentialStudent.value = data;
};
//请求潜在学员并渲染
